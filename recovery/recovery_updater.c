...
Value * VerifyBasebandFn(const char *name, State *state, int argc, Expr *argv[]) {
    ...
    uiPrintf(state, "ERROR: It appears you are running an unsupported baseband. Please visit   http://bit.ly/303shCMBaseband   to learn how to update.");
    return StringValue(strdup("0"));
}

void Register_librecovery_updater_303sh() {
    RegisterFunction("303sh.verify_baseband", VerifyBasebandFn);
}
